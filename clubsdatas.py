from matchpredictions.models import League, Club
...
for league_name, info in LEAGUES_DATA.items():
...
for club in CLUBS_DATA:
    league = League.objects.get(name=club["league_name"])
    ...
    if created:
        print(f"✅ Added club: {club_obj.name} ({league.name})")
    else:
        print(f"ℹ️ Club already exists: {club_obj.name}")

print("\n🎉 All leagues and clubs successfully imported!")
